const fs = require('fs');
const path = 'supabase/migrations/20260807233000_fix_cart_order_transaction_type_stamp.sql';
const helpers = fs.readFileSync(path, 'utf8');
...
fs.writeFileSync(path, helpers.trimEnd() + '\n' + patch);
console.log('appended, size', fs.statSync(path).size);
