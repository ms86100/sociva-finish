...
-- Round 1 Audit: Fix fn_enqueue_order_status_notification
-- 1. Dynamic terminal status check (replaces hardcoded list)
-- 2. Fix seller logo column (profile_image_url) and payload key (seller_logo_url)
...
BEGIN
  ...
  -- Fix 2: Use profile_image_url instead of logo_url
  ...
  -- Insert buyer notification (Fix 2: use seller_logo_url key)
  ...
  -- Fix 1: Dynamic terminal status check from category_status_flows
  SELECT EXISTS (
    SELECT 1 FROM public.category_status_flows
    WHERE status_key = NEW.status AND is_terminal = true
  ) INTO v_is_terminal;

  -- Fallback safety net for core terminal statuses
