...
-- Fix fn_enqueue_new_order_notification to handle auto-accepted orders (status='preparing' at INSERT)
...
-- Fix fn_enqueue_order_status_notification INSERT branch for auto-accepted orders
...
BEGIN
  ...
  -- NOTIFY BUYER
  IF _is_auto_accepted THEN
    -- Auto-accepted: buyer gets confirmation that order is already accepted
    INSERT INTO notification_queue (user_id, title, body, type, payload)
    VALUES (NEW.buyer_id, '✅ Order Confirmed!', 'Your order has been accepted and is being prepared by ' || COALESCE(_seller_name, 'the seller') || '.', 'order_status',
      jsonb_build_object('order_id',NEW.id,'new_status',NEW.status::text,'target_role','buyer','auto_accepted',true));
  ELSIF _flow.id IS NOT NULL AND _flow.notify_buyer THEN
  ...
  IF _seller_user_id IS NOT NULL THEN
    ...
    ELSIF TG_OP = 'INSERT' AND _is_auto_accepted THEN
      -- Auto-accepted: seller still gets notified via fn_enqueue_new_order_notification, skip duplicate here
      NULL;
