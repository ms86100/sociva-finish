...
-- ============================================================
-- Bug 1 + Bug 5: Add buyer notification & fix seller action in create_multi_vendor_orders
-- Bug 3: Fix food_beverages/self_fulfillment notification settings
-- ============================================================
...
begin
  ...
  loop
    ...
    -- Resolve seller name for notifications
    ...
    if not found then
      ...
      -- ============================================================
      -- NEW ORDER: insert items, payment record, notify seller + buyer
      -- ============================================================
      for _item in select * from json_array_elements(_seller_group->'items')
      loop
      ...
      -- Seller notification (Bug 5: added 'action' to payload)
      ...
      if _seller_user_id is not null then
        ...
        values (
          ...
          jsonb_build_object('orderId', _order_id::text, 'status', 'placed', 'type', 'order', 'action', 'View Order')
        );
      end if;

      -- Bug 1: Buyer confirmation notification
      insert into public.notification_queue (user_id, type, title, body, reference_path, payload)
      values (
        _buyer_id, 'order_status',
        '🛒 Order Placed!',
        'Your order from ' || coalesce(_seller_name, 'the seller') || ' has been placed.',
        '/orders/' || _order_id::text,
        jsonb_build_object('orderId', _order_id::text, 'status', 'placed', 'type', 'order_status', 'action', 'Track Order')
      );
    end if;
