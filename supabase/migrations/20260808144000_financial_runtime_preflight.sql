-- Read-only deployment gate for financial Edge Functions.
-- Safe to deploy before the finance schema: missing objects report not-ready.
...
COMMENT ON FUNCTION public.financial_runtime_preflight() IS
  'Read-only Edge deployment gate. Missing finance objects fail closed without enabling money movement.';
