...
-- =============================================
-- PHASE 1: Cross-Society Analytics Columns
-- =============================================
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS is_cross_society boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS buyer_society_id uuid REFERENCES public.societies(id),
  ADD COLUMN IF NOT EXISTS seller_society_id uuid REFERENCES public.societies(id),
  ADD COLUMN IF NOT EXISTS distance_km numeric;

-- Index for analytics queries
CREATE INDEX IF NOT EXISTS idx_orders_cross_society ON public.orders (is_cross_society) WHERE is_cross_society = true;
CREATE INDEX IF NOT EXISTS idx_orders_buyer_society ON public.orders (buyer_society_id);
CREATE INDEX IF NOT EXISTS idx_orders_seller_society ON public.orders (seller_society_id);

-- =============================================
-- PHASE 2: Geo Index on Societies
-- =============================================
CREATE INDEX IF NOT EXISTS idx_societies_lat_lng ON public.societies (latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;

-- =============================================
-- PHASE 3: Transaction-Safe Order Splitting RPC
-- =============================================
-- This function atomically creates orders for ALL sellers in a single transaction.
-- If ANY insert fails, the entire transaction rolls back → zero orphan orders.
...
CREATE OR REPLACE FUNCTION public.create_multi_vendor_orders(
  ...
  -- Each element: { "seller_id": "uuid", "subtotal": 123, "items": [{"product_id":"uuid","product_name":"x","quantity":1,"unit_price":10}] }
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
