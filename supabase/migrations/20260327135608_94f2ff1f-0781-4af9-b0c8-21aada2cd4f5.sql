...
declare
  ...
  -- New: product validation variables
  _product_id uuid;
  _product_row record;
...
begin
  ...
  -- ═══ AUTHORITATIVE PRODUCT VALIDATION (replaces client-side checks) ═══
  ...
  loop
    ...
    loop
      ...
      -- Check price drift (> 1 paisa tolerance)
  ...
  -- Return structured errors for any validation failure
  ...
  -- ═══ ORDER CREATION (using server-verified prices) ═══
  _group_count := 0;
  for _seller_group in select * from json_array_elements(_seller_groups)
  loop
    _total := 0;
    ...
    for _item in select * from json_array_elements(_seller_group->'items')
    loop
      -- Use server-verified price for financial calculations
      _product_id := (_item->>'product_id')::uuid;
    ...
    loop
      _product_id := (_item->>'product_id')::uuid;
      -- Use server price for order_items too
      select price into _client_price from public.products where id = _product_id;
