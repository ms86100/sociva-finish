-- Fix notification_queue schema mismatch in support functions.
-- Live schema uses (type, reference_path, payload). Old code wrote (action_type, action_id, priority)
-- which causes 42703 errors and blocks ticket creation + seller notification.
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      jsonb_build_object('ticket_id', v_ticket.id, 'order_id', v_ticket.order_id, 'priority', 'high', 'sla_breached', true)
    );
  END LOOP;
END;
$function$;

CREATE OR REPLACE FUNCTION public.fn_create_support_ticket(p_order_id uuid, p_issue_type text, p_issue_subtype text, p_description text, p_evidence_urls text[])
...
BEGIN
  ...
  VALUES (
    ...
    'support_ticket',
    '/support/' || v_ticket.id::text,
    jsonb_build_object('ticket_id', v_ticket.id, 'order_id', p_order_id, 'issue_type', p_issue_type, 'priority', 'high')
  );
