...
begin
  -- Idempotency check: one checkout can create multiple seller orders,
  -- so persisted row keys are namespaced as <base_key>:<seller_id>.
  if _idempotency_key is not null then
    ...
    where o.buyer_id = _buyer_id
      ...
      and (
        o.idempotency_key = _idempotency_key
        or o.idempotency_key like _idempotency_key || ':%'
      );
