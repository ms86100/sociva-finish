-- Persist multi-image gallery through create/update RPCs (image_url + secondary_images).

CREATE OR REPLACE FUNCTION public.save_product_with_service(p_product jsonb, p_service jsonb DEFAULT NULL::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_product ? 'secondary_images' AND jsonb_typeof(p_product->'secondary_images') = 'array' THEN
    SELECT COALESCE(array_agg(elem), ARRAY[]::text[])
    INTO v_secondary
    FROM jsonb_array_elements_text(p_product->'secondary_images') AS elem;
  ELSE
    v_secondary := v_row.secondary_images;
  END IF;

  -- Cap at 4 secondary URLs (primary lives in image_url)
  IF v_secondary IS NOT NULL AND cardinality(v_secondary) > 4 THEN
    v_secondary := v_secondary[1:4];
  END IF;

  INSERT INTO public.products (
    seller_id, name, description, price, mrp, image_url, secondary_images, category, is_veg,
    is_available, is_bestseller, is_recommended, is_urgent, listing_type,
    prep_time_minutes, approval_status, action_type, contact_phone,
    stock_quantity, low_stock_threshold, specifications, accepts_preorders,
    lead_time_hours, subcategory_id, updated_while_pending, tags, cuisine_type
  ) VALUES (
    v_seller_id,
...
CREATE OR REPLACE FUNCTION public.update_product_with_service(p_product_id uuid, p_product jsonb, p_service jsonb DEFAULT NULL::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF p_product ? 'secondary_images' AND jsonb_typeof(p_product->'secondary_images') = 'array' THEN
    ...
    INTO v_secondary
    FROM jsonb_array_elements_text(p_product->'secondary_images') AS elem;
    IF cardinality(v_secondary) > 4 THEN
      v_secondary := v_secondary[1:4];
    END IF;
  END IF;
  ...
  UPDATE public.products SET
    ...
    secondary_images = CASE
      WHEN p_product ? 'secondary_images' AND jsonb_typeof(p_product->'secondary_images') = 'array' THEN
        NULLIF(v_secondary, ARRAY[]::text[])
      WHEN p_product ? 'secondary_images' THEN NULL
