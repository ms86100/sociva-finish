...
-- Layer 3: Server-side stock validation in create_multi_vendor_orders
-- Layer 4: Cart items trigger to enforce stock ceiling

-- Layer 4: Trigger on cart_items
...
DROP TRIGGER IF EXISTS trg_enforce_cart_stock ON public.cart_items;
CREATE TRIGGER trg_enforce_cart_stock
  BEFORE INSERT OR UPDATE ON public.cart_items
  FOR EACH ROW EXECUTE FUNCTION public.enforce_cart_stock_limit();

-- Layer 3: Update create_multi_vendor_orders with stock validation
...
begin
  ...
  -- Pre-validate stock for ALL items before creating any orders
