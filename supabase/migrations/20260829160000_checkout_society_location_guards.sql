-- Checkout hardening: buyer society + seller society + resolvable seller location.

CREATE OR REPLACE FUNCTION public.seller_has_resolvable_location(p_seller_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE sp.id = p_seller_id
      AND (
        public.buyer_coordinates_are_valid(sp.latitude, sp.longitude)
        OR public.buyer_coordinates_are_valid(s.latitude::double precision, s.longitude::double precision)
      )
  );
$$;

GRANT EXECUTE ON FUNCTION public.seller_has_resolvable_location(uuid) TO anon, authenticated, service_role;

-- Patch create_multi_vendor_orders: fail fast before side-effects when parties lack society/location.
...
begin
  ...
  if _society_id is null then
    return json_build_object(
      'success', false,
      'error', 'buyer_society_required',
      'message', 'Link your account to a society in Profile settings before placing an order.'
    );
  end if;
  ...
  loop
    ...
    select sp.user_id, sp.business_name, sp.society_id,
    ...
    if _seller_society_id is null then
      return json_build_object(
        'success', false,
        'error', 'seller_society_required',
        'message', 'Store "' || COALESCE(_seller_name, 'Unknown') || '" is not linked to a society.',
        'seller', COALESCE(_seller_name, _seller_id::text)
      );
    end if;

    if not public.seller_has_resolvable_location(_seller_id) then
      return json_build_object(
        'success', false,
        'error', 'seller_location_required',
        'message', 'Store "' || COALESCE(_seller_name, 'Unknown') || '" has no location set.',
        'seller', COALESCE(_seller_name, _seller_id::text)
      );
    end if;
