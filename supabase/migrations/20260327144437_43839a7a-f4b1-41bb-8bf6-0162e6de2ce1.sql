...
-- ═══════════════════════════════════════════════════════════════════════════
-- CONSOLIDATE create_multi_vendor_orders: drop old overload, fix product_name,
-- fix date/time casting, single authoritative implementation
-- ═══════════════════════════════════════════════════════════════════════════

-- 1. Drop the OLD overload (uuid, uuid, json with _society_id, _payment_mode)
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(
  uuid, uuid, json, text, text, text, uuid, double precision, double precision, text, text, text, uuid[]
);

-- Also drop any other old signatures that may linger
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(
  _buyer_id uuid, _society_id uuid, _seller_groups json,
  _delivery_address text, _notes text, _payment_mode text,
  _delivery_address_id uuid, _delivery_lat double precision, _delivery_lng double precision,
  _idempotency_key text, _scheduled_date text, _scheduled_time_start text, _preorder_seller_ids uuid[]
);

-- 2. Drop the current canonical version so we can recreate cleanly
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(
...
-- 3. Recreate the ONE canonical implementation
