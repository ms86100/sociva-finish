...
-- GAP 1: Create delivery_assignments for seller self-delivery orders
-- New trigger: when seller marks order as picked_up and delivery_handled_by = 'seller'
...
BEGIN
  ...
  -- Only when transitioning to picked_up
  ...
  -- Calculate initial ETA from seller to buyer using haversine
  -- Get seller location from their profile
  ...
  IF _seller_lat IS NOT NULL AND _seller_lng IS NOT NULL AND _dest_lat IS NOT NULL AND _dest_lng IS NOT NULL THEN
    ...
    -- ETA: distance * 1.3 road factor / 15 km/h default speed, minimum 2 min
  ...
  VALUES (
    NEW.id, COALESCE(_buyer_society, NEW.buyer_society_id), COALESCE(NEW.delivery_fee, 0), _idempotency,
...
CREATE TRIGGER trg_create_seller_delivery_assignment
  ...
  FOR EACH ROW EXECUTE FUNCTION public.trg_create_seller_delivery_assignment();
