...
BEGIN
  ...
  SELECT
    ...
    public.haversine_km(_lat, _lng, s.latitude::double precision, s.longitude::double precision) AS distance_km,
  ...
  WHERE sp.verification_status = 'approved'
    ...
    AND public.haversine_km(_lat, _lng, s.latitude::double precision, s.longitude::double precision) <= LEAST(_radius_km, COALESCE(sp.delivery_radius_km, _radius_km))
