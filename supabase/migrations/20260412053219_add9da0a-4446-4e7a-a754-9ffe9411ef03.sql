...
-- Batch RPC: resolve all sections' products for a banner in one call
...
BEGIN
  ...
  FROM (
    ...
    WHERE bs.banner_id = p_banner_id
      ...
      -- Seller participation enforcement
      AND (
        NOT EXISTS (SELECT 1 FROM festival_seller_participation fsp2 WHERE fsp2.banner_id = p_banner_id AND fsp2.seller_id = sp.id)
        OR EXISTS (
          SELECT 1 FROM festival_seller_participation fsp3
          WHERE fsp3.banner_id = p_banner_id AND fsp3.seller_id = sp.id AND fsp3.opted_in = true
        )
      )
      -- Society filtering: if society provided, seller must serve that society
      AND (
