...
BEGIN
  ...
  -- Set OTP verified flag FIRST so both assignment and order triggers pass
  PERFORM set_config('app.otp_verified', 'true', true);

  -- Step 1: Mark delivery assignment as delivered
  UPDATE public.delivery_assignments
  SET
    status = 'delivered',
    delivered_at = now(),
  ...
  -- Step 2: Transition order to 'delivered'
