...
-- Update create_multi_vendor_orders to resolve transaction_type from canonical mapping
...
begin
  ...
  -- Validation pass: products, prices, stock, payment config, AND checkout_mode
  ...
  loop
    ...
    loop
      ...
      -- Validate checkout_mode is cart-compatible
  ...
  -- Hard-fail if non-cart items are in a cart order
  ...
  loop
    ...
    -- Payment method validation per seller per fulfillment type
  ...
  loop
    ...
    -- Resolve transaction_type from canonical mapping + fulfillment context
    _resolved_tx_type := 'cart_purchase'; -- default for cart
    IF _fulfillment_type = 'self_pickup' THEN
      _resolved_tx_type := 'self_fulfillment';
    ELSIF _fulfillment_type = 'delivery' AND _delivery_handled_by = 'seller' THEN
      _resolved_tx_type := 'seller_delivery';
    ELSIF _fulfillment_type = 'delivery' AND _delivery_handled_by = 'platform' THEN
      _resolved_tx_type := 'cart_purchase';
    END IF;
