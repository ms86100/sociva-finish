...
USING (
  (
    -- Approved products from approved sellers (marketplace-open)
...
CREATE OR REPLACE FUNCTION public.search_sellers_by_location(
  ...
  _exclude_society_id uuid DEFAULT NULL
)
RETURNS TABLE(
...
BEGIN
  ...
  WHERE sp.verification_status = 'approved'
    ...
    -- Community gating: coordinate-based radius overrides society boundary
    -- Commercial sellers and sell_beyond sellers always pass
    -- Society-resident sellers visible if within radius (already guaranteed by haversine check above)
    -- Only block society-resident sellers who are NOT within radius AND NOT sell_beyond
    -- (The haversine filter above already ensures within-radius, so we only need sell_beyond for edge cases)
    AND (
      ...
      -- Within radius: any seller who passed the haversine filter is visible
      OR public.haversine_km(_lat, _lng,
          COALESCE(sp.latitude, s.latitude::double precision),
          COALESCE(sp.longitude, s.longitude::double precision)
        ) <= LEAST(_radius_km, COALESCE(sp.delivery_radius_km, _radius_km))
    )
