-- Treat Mon/mon/MONDAY as the same operating day so seeded and form-saved
-- lowercase days do not mark a store Closed today.
...
BEGIN
  ...
  END IF;
  IF p_days IS NOT NULL AND array_length(p_days, 1) > 0
     AND NOT EXISTS (
       SELECT 1 FROM unnest(p_days) AS d
       WHERE lower(left(d, 3)) = lower(left(v_current_day, 3))
     )
  THEN
