-- Drop all 4 overloaded versions of create_multi_vendor_orders
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(uuid, json, text, text, text, text, numeric, text, text, numeric, boolean, numeric, text, uuid, double precision, double precision, text, text, text, uuid[]);
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(uuid, json, text, text, text, text, numeric, text, text, numeric, boolean, numeric, text, uuid, double precision, double precision, text, date, time);
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(uuid, json, text, text, text, text, numeric, text, text, numeric, boolean, numeric, text, uuid, double precision, double precision, text);
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(uuid, json, text, text, text, text, text, uuid, double precision, double precision, numeric, text, numeric, text);

-- Create single canonical function with all parameters
...
begin
  ...
  -- Payment status: online payments → payment_pending, COD → placed
  ...
  -- Idempotency: check for existing orders
  if _idempotency_key is not null then
  ...
  loop
    ...
    loop
      ...
      SELECT p.stock_quantity, p.name INTO _available_stock, _product_name_check
      ...
      IF _available_stock IS NOT NULL AND _requested_qty > _available_stock THEN
        RETURN json_build_object(
          ...
          'product_name', COALESCE(_item->>'product_name', _product_name_check, 'Item'),
          'available', _available_stock,
          'requested', _requested_qty
        );
      END IF;
    end loop;
  end loop;

  -- ===== SELLER AVAILABILITY & DISTANCE CHECKS =====
  for _seller_group in select * from json_array_elements(_seller_groups)
  loop
    _seller_id := (_seller_group->>'seller_id')::uuid;

    select sp.business_name,
           public.compute_store_status(sp.availability_start, sp.availability_end, sp.operating_days, coalesce(sp.is_available, true)),
  ...
  -- Auto-cancel timer for online payments only
  ...
  loop
    ...
    -- Scheduled date: only apply to pre-order sellers
    ...
    if not found then
      ...
      -- New order: insert items, deduct stock, create payment record, notify seller
      for _item in select * from json_array_elements(_seller_group->'items')
      loop
      ...
      -- Seller notification (only for placed orders, not payment_pending)
      if _effective_status = 'placed' then
        ...
        if _seller_user_id is not null then
          ...
          -- Add scheduled time info for pre-orders
