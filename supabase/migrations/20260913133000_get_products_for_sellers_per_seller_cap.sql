-- Fair product attachment for marketplace discovery:
-- each seller in the batch gets up to N products so large catalogs cannot starve others.
-- Same signature and return columns as before; gates unchanged.
...
AS $function$
DECLARE
  v_per_seller integer;
BEGIN
  v_per_seller := LEAST(12, GREATEST(3, COALESCE(_limit, 20)));
