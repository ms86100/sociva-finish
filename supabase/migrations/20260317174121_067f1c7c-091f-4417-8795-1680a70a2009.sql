...
-- Update create_multi_vendor_orders to set delivery_handled_by based on seller's fulfillment_mode
  ...
  loop
    ...
    if _fulfillment_type = 'delivery' then
      ...
      if _seller_fulfillment_mode in ('seller_delivery', 'pickup_and_seller_delivery') then
        ...
        -- Default: seller handles delivery (since platform delivery is coming soon)
        _delivery_handled_by := 'seller';
      end if;
    end if;
