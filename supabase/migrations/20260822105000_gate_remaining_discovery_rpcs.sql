-- Close leftover discovery RPCs that still returned unactivated or ungated sellers.

CREATE OR REPLACE FUNCTION public.get_user_frequent_products(
  _user_id uuid,
...
BEGIN
  ...
  WHERE o.buyer_id = _user_id
    ...
    AND public.seller_is_eligible_for_discovery(p.seller_id)
...
BEGIN
  ...
  WHERE public.seller_is_eligible_for_discovery(sp.id)
