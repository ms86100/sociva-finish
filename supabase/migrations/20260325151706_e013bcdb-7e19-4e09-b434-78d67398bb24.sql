...
begin
  ...
  -- Recalculate discount server-side; never trust client _coupon_discount
  IF _resolved_coupon_id IS NOT NULL THEN
    -- Compute cart total from seller_groups to validate min_order_amount
    _cart_total := 0;
    ...
    LOOP
      ...
      LOOP
        _cart_total := _cart_total + ((_item->>'unit_price')::numeric * (_item->>'quantity')::int);
      END LOOP;
    END LOOP;
    ...
    THEN
      ...
      _coupon_discount := 0;
    ELSE
      -- Validate seller ownership: coupon seller must be in this order
      IF NOT EXISTS (
        SELECT 1 FROM json_array_elements(_seller_groups) sg
        WHERE (sg->>'seller_id')::uuid = _coupon_row.seller_id
      ) THEN
        _resolved_coupon_id := NULL;
        _coupon_discount := 0;
      ELSE
        ...
        IF _user_redemption_count >= _coupon_row.per_user_limit THEN
          _resolved_coupon_id := NULL;
          _coupon_discount := 0;
        ELSIF _coupon_row.min_order_amount IS NOT NULL AND _cart_total < _coupon_row.min_order_amount THEN
          _resolved_coupon_id := NULL;
          _coupon_discount := 0;
        ELSE
          -- Recalculate discount server-side (ignore client value entirely)
          IF _coupon_row.discount_type = 'percentage' THEN
            _server_discount := (_cart_total * LEAST(_coupon_row.discount_value, 100)) / 100;
            IF _coupon_row.max_discount_amount IS NOT NULL THEN
              _server_discount := LEAST(_server_discount, _coupon_row.max_discount_amount);
            END IF;
          ELSE
            -- flat discount
            _server_discount := LEAST(_coupon_row.discount_value, _cart_total);
          END IF;
  ...
  loop
    ...
    -- Resolve transaction_type at order creation (single source of truth)
