...
-- Dashboard EarningsSummary, SellerEarningsPage overview, and analytics AOV/revenue
-- must all use this definition (payout nets are separate and live under /seller/payouts).
...
BEGIN
  ...
  -- IST day / week (Sunday) / month boundaries as timestamptz
...
COMMENT ON FUNCTION public.get_seller_dashboard_kpis(uuid) IS
  'Seller dashboard KPIs. Settled GMV = completed|delivered|buyer_received AND payment_status <> refunded. Action needed excludes preparing/ready/in_transit. Fulfill minutes prefer delivered_at > status_changed_at > updated_at.';

CREATE OR REPLACE FUNCTION public.get_seller_order_board_counts(p_seller_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
