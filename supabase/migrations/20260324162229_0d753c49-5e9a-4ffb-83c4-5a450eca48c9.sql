...
-- Fix Issue 7: Update delivery assignment status to match order status during non-terminal OTP steps
...
BEGIN
  ...
  -- Get the IMMEDIATE next step and check if IT specifically requires delivery OTP
  ...
  -- Validate transition exists in transitions table
  SELECT EXISTS (
  ...
  -- FIX Issue 7: Sync delivery assignment status with the order's next step
  -- Terminal → 'delivered', non-terminal → use the next step's status_key as assignment status
  UPDATE public.delivery_assignments
  SET
    status = CASE
      WHEN _next_step_is_terminal THEN 'delivered'
      ELSE _next_step_status
    END,
