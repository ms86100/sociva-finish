-- Lock out delivery OTP after 5 wrong tries for 15 minutes.
-- The buyer still sees delivery_code. This only stops guessing.
...
BEGIN
  ...
  IF _assignment.otp_locked_until IS NOT NULL AND _assignment.otp_locked_until > now() THEN
    RAISE EXCEPTION 'Too many attempts. Try again in 15 minutes';
  END IF;

  IF _assignment.delivery_code != _delivery_code THEN
    _attempts := COALESCE(_assignment.otp_attempt_count, 0) + 1;
    IF _attempts >= 5 THEN
      UPDATE delivery_assignments
      SET otp_attempt_count = 0,
          otp_locked_until = now() + interval '15 minutes'
      WHERE id = _assignment.id;
      RAISE EXCEPTION 'Too many attempts. Try again in 15 minutes';
    END IF;
    UPDATE delivery_assignments
    SET otp_attempt_count = _attempts
    WHERE id = _assignment.id;
