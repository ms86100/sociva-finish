...
-- Add role-aware cancellation logic to fn_enqueue_order_status_notification
-- Detects buyer-initiated cancellations via rejection_reason and adjusts seller notification title
...
BEGIN
  ...
  -- ===== NEW: Role-aware cancellation title =====
  -- Detect buyer-initiated cancellation and adjust seller notification title
  IF NEW.status = 'cancelled' AND NEW.rejection_reason IS NOT NULL THEN
    IF NEW.rejection_reason ILIKE 'Cancelled by buyer%' OR NEW.rejection_reason ILIKE '%buyer cancelled%' THEN
  ...
  -- Buyer notification (with target_role = 'buyer')
  ...
  -- Seller notification (with target_role = 'seller')
