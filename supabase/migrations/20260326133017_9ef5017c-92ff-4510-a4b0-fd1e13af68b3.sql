...
-- Bug 9 Fix: Only set auto_cancel_at for non-COD orders
-- COD orders don't need a payment confirmation step, so they should not be auto-cancelled.
-- The auto-cancel edge function targets orders with auto_cancel_at < now() and payment_status NOT IN (buyer_confirmed, paid).
-- Since COD orders have payment_status = 'pending', they were being auto-cancelled after 3 minutes.
...
begin
  ...
  -- Bug 9 fix: Only set auto_cancel_at for non-COD orders.
  -- COD orders are confirmed immediately and should NOT be auto-cancelled.
...
-- Also clear auto_cancel_at on any existing COD orders that were incorrectly set
UPDATE public.orders
SET auto_cancel_at = NULL
