...
-- Add idempotency support to create_multi_vendor_orders
-- Drop existing signature first to allow parameter change
DROP FUNCTION IF EXISTS public.create_multi_vendor_orders(uuid, json, text, text, text, text, text, uuid, double precision, double precision, numeric, text, numeric);
...
begin
  ...
  if _idempotency_key is not null and btrim(_idempotency_key) <> '' then
    ...
    if _existing_ids is not null and array_length(_existing_ids, 1) > 0 then
  ...
  loop
    ...
    insert into public.orders (
      ...
      delivery_handled_by, idempotency_key
    )
    values (
      _order_id, _buyer_id, _seller_id, _society_id,
      ...
      CASE WHEN _group_count = 1 THEN _idempotency_key ELSE NULL END
    );
  ...
  -- CRITICAL FIX: Do NOT delete cart items here.
  -- Cart must only be cleared client-side AFTER payment is confirmed.

  return json_build_object('success', true, 'order_ids', to_json(_order_ids), 'deduplicated', false);
end;
$$;
