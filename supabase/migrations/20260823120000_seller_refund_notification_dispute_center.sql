-- Seller "Refund requested" notifications must open Disputes & Refunds
-- (/seller?tab=refunds), not buyer My Orders (/orders/{id}).
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE order_id = p_order_id AND status NOT IN ('rejected', 'completed')
  ) THEN
...
-- Heal existing seller refund alerts still pointing at buyer My Orders.
UPDATE public.notification_queue nq
SET reference_path = CASE
  WHEN COALESCE(nq.payload->>'refundId', nq.payload->>'refund_id') IS NOT NULL
    THEN '/seller?tab=refunds&refundId=' || COALESCE(nq.payload->>'refundId', nq.payload->>'refund_id')
  ELSE '/seller?tab=refunds'
END
WHERE nq.title = 'Refund requested'
  AND COALESCE(nq.payload->>'target_role', '') = 'seller'
  AND COALESCE(nq.payload->>'status', '') IN ('refund_requested', 'refund_request')
  AND (
    nq.reference_path IS NULL
    OR nq.reference_path LIKE '/orders/%'
    OR nq.reference_path = '/orders'
  );

UPDATE public.user_notifications un
SET
  action_url = CASE
    WHEN COALESCE(un.data->>'refundId', un.data->>'refund_id', un.payload->>'refundId', un.payload->>'refund_id') IS NOT NULL
      THEN '/seller?tab=refunds&refundId=' || COALESCE(
        un.data->>'refundId', un.data->>'refund_id', un.payload->>'refundId', un.payload->>'refund_id'
      )
    ELSE '/seller?tab=refunds'
  END,
  reference_path = CASE
...
WHERE un.title = 'Refund requested'
  ...
  AND COALESCE(
    ...
    un.payload->>'status',
    ''
  ) IN ('refund_requested', 'refund_request')
  AND (
    un.action_url IS NULL
    OR un.action_url LIKE '/orders/%'
    OR un.action_url = '/orders'
    OR un.reference_path LIKE '/orders/%'
    OR un.reference_path = '/orders'
  );
