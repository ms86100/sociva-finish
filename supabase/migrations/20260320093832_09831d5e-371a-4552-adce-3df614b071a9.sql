...
BEGIN
  ...
  WHERE csf.status_key = NEW.status::text
    ...
    AND csf.parent_group = COALESCE(v_parent_group, 'food_and_beverage')
  LIMIT 1;
  ...
  IF v_notify_buyer AND v_title IS NOT NULL AND v_title <> '' THEN
    -- Dedup: skip if same status notification already queued in last 60s
    ...
    IF v_existing_id IS NULL THEN
      INSERT INTO public.notification_queue (
        ...
        NEW.id,
        'order',
        CASE WHEN v_silent THEN 'silent' ELSE 'high' END,
        jsonb_build_object(
          'orderId', NEW.id,
          'status', NEW.status,
  ...
  -- ── Seller notification ──
  IF v_notify_seller AND v_seller_title IS NOT NULL AND v_seller_title <> '' THEN
    SELECT id INTO v_existing_id
      FROM public.notification_queue
     WHERE user_id = (SELECT user_id FROM public.seller_profiles WHERE id = v_seller_id)
       AND reference_id = NEW.id
    ...
    IF v_existing_id IS NULL THEN
      INSERT INTO public.notification_queue (
        ...
        (SELECT user_id FROM public.seller_profiles WHERE id = v_seller_id),
        v_seller_title,
        v_seller_body,
        'order_update',
        NEW.id,
  ...
  -- ── Terminal: clean up live-activity tokens ──
