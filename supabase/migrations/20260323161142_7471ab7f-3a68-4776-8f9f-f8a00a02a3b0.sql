...
-- Update trg_create_seller_delivery_assignment to be workflow-driven
-- Replace hardcoded 'picked_up' check with dynamic is_transit lookup from category_status_flows
...
BEGIN
  ...
  -- Only for seller-handled delivery orders
  ...
  -- Resolve parent_group from seller profile
  SELECT sp.primary_group INTO _v_parent_group
  FROM seller_profiles sp
  WHERE sp.id = NEW.seller_id;

  -- Workflow-driven: check if the new status is an is_transit step
  SELECT EXISTS (
  ...
  -- Only create assignment on transit steps (replaces hardcoded 'picked_up' check)
  ...
  -- Generate 4-digit delivery code inline
