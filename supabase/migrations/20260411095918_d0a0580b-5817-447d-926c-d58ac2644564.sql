...
BEGIN
  ...
  SELECT o.buyer_id, o.society_id, sp.business_name, sp.society_id
    INTO _buyer_id, _order_society_id, _seller_name, _seller_society_id
  FROM public.orders o
  LEFT JOIN public.seller_profiles sp ON sp.id = o.seller_id
  WHERE o.id = NEW.order_id;

  SELECT p.flat_number, p.society_id
    INTO _buyer_flat, _buyer_society_id
  FROM public.profiles p
  WHERE p.id = _buyer_id;

  _resolved_society_id := COALESCE(
    NEW.society_id,
    _order_society_id,
    _buyer_society_id,
    _seller_society_id
